...
class GoogleBookSearchRepository(
        private val googleBookSearchService: GoogleBooksService
    ...
    override fun search(term: String): Observable<List<Book>> {
        return googleBookSearchService.search(term)
                .map { response: GoogleBooksResponse ->
                    response.items.map {
                        Book(it.volumeInfo.title!!, it.volumeInfo.authors.first(), it.volumeInfo.imageLinks?.thumbnail!!)
                    }
                }
    ...
    companion object {
        fun create(): BookSearchRepository {
            ...
            return GoogleBookSearchRepository(googleBooksService)
        }
    }
}